...
namespace ERM.UI
{
    ...
    public class WriteDog
    {
        ...
        public static void MakeFile(string FileName, byte[] PubData)//产生升级包文件
        {
            FileStream fs = new FileStream(FileName, FileMode.Create);//创建文件名为FileName的升级包文件
            fs.Write(PubData, 0, PubData.Length);//开始写入
            fs.Flush();//清空缓冲区、关闭流
            fs.Close();
        }

        public static byte[] GetFile(string FileName) //获取升级包文件，用于升级
        {
        ...
        public static string DoReader()
        {
            ...
            try
            {
                ...
                //打开Rockey3, 打开找到的第一把Rockey3。
                //第二个参数为打开相同VendorID的Rockey3的索引，从1开始。当只插入一把锁时，这里给1；当插入多把锁时，这里分别给1，2 ...，表示打开不同的Rockey3。
                ...
                //控制LED灯，亮，灭，闪烁------1:ON 2:OFF 3:WINK
                iRes = RY3_LEDControl(hHandle, 1);

                //设定客户号(输入不大于250字节的种子码, 输出8字节字符串形式的客户号)        
                //iRes = RY3_SetVendorID(hHandle, sVendorSeed, 6, bVendorID);
            ...
            //        //iRes = RY3_Write(hHandle, 0, strGuid, strGuid.Length);
            ...
            //            //strtemp = strtemp + "," + ExpireDate.ToString("yyyy-MM-dd") + "," + DateTime.Now.ToString("yyyy-MM-dd");//日期标准化
            //            strtemp = str[0] + "," + str[1] + "," + ExpireDate.ToString("yyyy-MM-dd") + "," + DateTime.Now.ToString("yyyy-MM-dd");//日期标准化

            //            if (!String.IsNullOrEmpty(strtemp))
            //            {//写Key
            //                //iRes = RY3_Write(hHandle, 0, strtemp, strtemp.Length);
            ...
            //catch
            //{
            //    //return "读取USB-Key失败，请与供应商联系！";// false;
            //    RY3_Update(hHandle, null, strtemp.Length + 10);
            //    string strGuid = Guid.NewGuid().ToString();
            //    strGuid = strGuid + "," + MD5Encrypt(strGuid) + ",2020-01-01," + System.DateTime.Now.ToString("yyyy-MM-dd");
            //    //iRes = RY3_Write(hHandle, 0, strGuid, strGuid.Length);
            ...
            //{
            //关闭Rockey3。
            //第二个参数表示，是否在关闭时将锁内的状态恢复为没有验证密码的状态。true表示将锁的状态恢复为没有密码的状态，false表示保持原来的状态。
            //上面的说明是针对锁没有拔出的情况，锁拔出后安全状态都会被重置为没有验证。
            //iRes = RY3_Close(hHandle, true);
            // Console.ReadKey();
            //}            
