...
namespace ERM.UI
{
    ...
    public class MyCommon
    {
        ...
        ///  4 用地规划许可证号
        /// </param>
        public static void ShowTextBind(System.Windows.Forms.Label lbl_showLable,
            string lable_flg)
        {
            switch (lable_flg)
            {
        ...
        public static void GetFileCount(string sourceDirName, ref bool check_flg, ref int fileCount)
        {
            if (check_flg)
            {
                ...
                else
                {
                    check_flg = false;
                    fileCount = 0;
                    MessageBox.Show("提示：文件夹路径错误：'" + sourceDirName + "' \r\r\n 【温馨提示：文件路径不存在或文件夹尾部可能包含特殊字符或全角字符 例如：空格,星号...】",
            ...
            string operationType, string fullFilePath)
        {
            if (File.Exists(fullFilePath))
            {
                ...
                //FileStream st = new FileStream(fullFilePath,FileMode.Open,FileAccess.Read);
                //mdl.FileSize = (Int32)st.Length;
                //st.Close();
                //st.Dispose();
        ...
        public static void DaoChuPrjectInfo(string projNo, string projName)
        {
            if (TXMessageBoxExtensions.Question("提示：确定导出：" + projName + " 项目信息吗？") == DialogResult.OK)
            {
                ...
                XmlElement projBaseInfo = To_JavaXml.CreateElement("projBaseInfo");
                root.AppendChild(projBaseInfo);
                ...
                #region
                //CreateElement("projName", projectFactory.ProjectDetail["constructionprojectname"].ToString(), To_JavaXml, projBaseInfo);//项目名称
                //CreateElement("projLocation", projectFactory.ProjectDetail["constructionprojectadd"].ToString(), To_JavaXml, projBaseInfo);//地址
                //CreateElement("projZoneCode", projectFactory.ProjectDetail["respective"].ToString(), To_JavaXml, projBaseInfo);//所属城区

                //if (projectFactory.ProjectDetail["jsdw_mc"] is System.String[])
                //{
                //    string Svalue = "";
                //    foreach (string values in (projectFactory.ProjectDetail["jsdw_mc"] as System.String[]))
                //    {
                ...
                //    }
                //    CreateElement("developmentsOrgName", Svalue, To_JavaXml, projBaseInfo);//建设单位
                //}
                //else
                //{
                //    CreateElement("developmentsOrgName", projectFactory.ProjectDetail["jsdw_mc"].ToString(), To_JavaXml, projBaseInfo);//建设单位
                //}

                //if (projectFactory.ProjectDetail["kcdw_mc"] is System.String[])
                //{
                //    string Svalue = "";
                //    foreach (string values in (projectFactory.ProjectDetail["kcdw_mc"] as System.String[]))
                //    {
                //        if (!string.IsNullOrWhiteSpace(values))
                //            Svalue += values + "，";
                //    }
                ...
                //        Svalue = Svalue.Remove(Svalue.Length - 1);
                //    }
                //    CreateElement("reconnaissanceOrgSid", Svalue, To_JavaXml, projBaseInfo);//勘察单位
                //}
                //else
                //{
                //    CreateElement("reconnaissanceOrgSid", projectFactory.ProjectDetail["kcdw_mc"].ToString(), To_JavaXml, projBaseInfo);//勘察单位
                //}

                //CreateElement("initiationApprovalOrg", projectFactory.ProjectDetail["pzdw"].ToString(), To_JavaXml, projBaseInfo);//立项批准单位
                //CreateElement("initiationApprovalNo", projectFactory.ProjectDetail["pzh"].ToString(), To_JavaXml, projBaseInfo);//立项批准号
                ////CreateElement("totalConstructionArea", projectFactory.ProjectDetail["constructionareasum"].ToString(), To_JavaXml, projBaseInfo);//总建筑面积
                //CreateElement("totalBuildingNums", projectFactory.ProjectDetail["ds"].ToString(), To_JavaXml, projBaseInfo);//栋数
                //CreateElement("buildNature", projectFactory.ProjectDetail["projecttype_item"].ToString(), To_JavaXml, projBaseInfo);//建筑性质
                //CreateElement("totalLandUseArea", projectFactory.ProjectDetail["usesoiareasum"].ToString(), To_JavaXml, projBaseInfo);//总用地面积
                //CreateElement("totalLandCost", projectFactory.ProjectDetail["allcost"].ToString(), To_JavaXml, projBaseInfo);//总地价
                //CreateElement("landUseNo", project_MDL.ydpzcode, To_JavaXml, projBaseInfo);//用地批准书号
                //CreateElement("landUsePlanningNo", project_MDL.ghcode, To_JavaXml, projBaseInfo);//用地规划许可证号
                //CreateElement("volumeRate", projectFactory.ProjectDetail["volumefar"].ToString(), To_JavaXml, projBaseInfo);//容积率
                //CreateElement("buildDensity", projectFactory.ProjectDetail["buildingdensity"].ToString(), To_JavaXml, projBaseInfo);//建筑密度
                //CreateElement("virescenceRate", projectFactory.ProjectDetail["greenfar"].ToString(), To_JavaXml, projBaseInfo);//绿化率
                //CreateElement("overgroundParking", project_MDL.dstcw, To_JavaXml, projBaseInfo);//地上停车位
                //CreateElement("undergroundParking", project_MDL.dxtcw, To_JavaXml, projBaseInfo);//地下停车位
                //CreateElement("totalParking", project_MDL.ztcw, To_JavaXml, projBaseInfo);//总停车位
                ////CreateElement("totalEngBudget", projectFactory.ProjectDetail["projectsettlement"].ToString(), To_JavaXml, projBaseInfo);//总预算
                ////CreateElement("totalEngCost", projectFactory.ProjectDetail["projectcost"].ToString(), To_JavaXml, projBaseInfo);//总结算
                #endregion

                CreateElement("ConStructionProjectName", projectFactory.ProjectDetail["constructionprojectname"].ToString(), To_JavaXml, projBaseInfo);//项目名称
                CreateElement("ConstructionProjectAdd", projectFactory.ProjectDetail["constructionprojectadd"].ToString(), To_JavaXml, projBaseInfo);//地址
                CreateElement("Respective", projectFactory.ProjectDetail["respective"].ToString(), To_JavaXml, projBaseInfo);//所属城区
                CreateElement("ConstructionAreaSum","", To_JavaXml, projBaseInfo);//总建筑面积
                CreateElement("ProjectType", "", To_JavaXml, projBaseInfo);//建筑性质
                CreateElement("UseSoiAreaSum", projectFactory.ProjectDetail["usesoiareasum"].ToString(), To_JavaXml, projBaseInfo);//总用地面积
                CreateElement("AllCost", projectFactory.ProjectDetail["allcost"].ToString(), To_JavaXml, projBaseInfo);//总地价
                CreateElement("VolumeFar", projectFactory.ProjectDetail["volumefar"].ToString(), To_JavaXml, projBaseInfo);//容积率
                CreateElement("BuildingDensity", projectFactory.ProjectDetail["buildingdensity"].ToString(), To_JavaXml, projBaseInfo);//建筑密度
                CreateElement("GreenFar", projectFactory.ProjectDetail["greenfar"].ToString(), To_JavaXml, projBaseInfo);//绿化率
                CreateElement("ds", "", To_JavaXml, projBaseInfo);//栋数
                CreateElement("ProjectSettlement", projectFactory.ProjectDetail["projectsettlement"].ToString(), To_JavaXml, projBaseInfo);//总预算
                CreateElement("ProjectCost", projectFactory.ProjectDetail["projectcost"].ToString(), To_JavaXml, projBaseInfo);//总结算
                CreateElement("ItemFzr", projectFactory.ProjectDetail["itemfzr"].ToString(), To_JavaXml, projBaseInfo); //项目负责人
                CreateElement("pzdw", projectFactory.ProjectDetail["pzdw"].ToString(), To_JavaXml, projBaseInfo); //立项批准单位
                CreateElement("pzh", projectFactory.ProjectDetail["pzh"].ToString(), To_JavaXml, projBaseInfo); //批准文号
                CreateElement("CreateUnit", projectFactory.ProjectDetail["createunit"].ToString(), To_JavaXml, projBaseInfo);//建设单位
                CreateElement("xmreconnaissanceOrgSidtmp", projectFactory.ProjectDetail["kcdw_mc"].ToString(), To_JavaXml, projBaseInfo);//勘察单位
                //工程规划许可证
                CreateElement("xmghcode", projectFactory.ProjectDetail["ghcode"].ToString(), To_JavaXml, projBaseInfo);
                //设计单位
                CreateElement("xmdesignOrgSidtmp", projectFactory.ProjectDetail["sjdw_mc"].ToString(), To_JavaXml, projBaseInfo);
                //用地规划许可证
                CreateElement("xmydxkcode", projectFactory.ProjectDetail["ydxkcode"].ToString(), To_JavaXml, projBaseInfo);
                //用地批准书号
                CreateElement("xmydpzcode", projectFactory.ProjectDetail["ydpzcode"].ToString(), To_JavaXml, projBaseInfo);
                //监理单位
                CreateElement("xmsupervisionOrgSidtmp", projectFactory.ProjectDetail["jldw_mc"].ToString(), To_JavaXml, projBaseInfo);
                //施工单位许可证号
                CreateElement("xmsgcode", projectFactory.ProjectDetail["sgcode"].ToString(), To_JavaXml, projBaseInfo); 
                ...
                To_JavaXml.Save(System.IO.Path.Combine(tempPath, "projInfo.xml"));
            ...
            XmlDocument xDoc, XmlElement PElement)
        {
            ...
            //X_temp.InnerText = KeyValue;
            X_temp.SetAttribute("value", KeyValue);
            //XmlAttribute x_attribute = new XmlAttribute();
            //x_attribute.Name = "Value";
            //x_attribute.Value = "Value";
